...
const renderAdminPanel = () => {
    ...
    panel.innerHTML = `
        <h2 class="text-2xl sm:text-3xl font-bold text-primary mb-4 sm:mb-6">Administrator Dashboard</h2>
        
        <!-- Welcome Banner -->
        <div class="welcome-banner">
            <div class="welcome-scroll-container">
                <div class="welcome-scroll-text">
                    🎓 Welcome to DAV Public School NTPC Township, Jamthal — where learning meets innovation! 🚀
                </div>
            </div>
        </div>
        ...
        <div class="flex border-b border-gray-300 mb-6">
            <button data-tab-name="add-users" onclick="switchTab('add-users')" class="tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-white border-b-4 border-primary font-bold">
                <i data-lucide="user-plus" class="w-4 h-4 mr-2"></i> Add Users
            </button>
            <button data-tab-name="users" onclick="switchTab('users')" class="tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="users" class="w-4 h-4 mr-2"></i> Users Management (${teachers.length + students.length})
            </button>
            <button data-tab-name="notifications" onclick="switchTab('notifications')" class="tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="bell" class="w-4 h-4 mr-2"></i> Notifications (${notifications.length})
            </button>
            <button data-tab-name="marks-config" onclick="switchTab('marks-config')" class="tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="settings" class="w-4 h-4 mr-2"></i> Marks Configuration
            </button>
        </div>

        <!-- Add Users Section -->
        <div id="add-users-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            <div class="flex justify-between items-center mb-6 border-b pb-3">
                ...
                <div class="flex space-x-2">
                    <button type="button" onclick="debugAddUserForm()" class="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs hover:bg-gray-200">
                        <i data-lucide="bug" class="w-3 h-3 inline mr-1"></i>
                        Debug
                    </button>
                    <button type="button" onclick="testFormSubmission()" class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs hover:bg-blue-200">
            ...
            <div class="max-w-md mx-auto">
                <form id="add-user-form" onsubmit="event.preventDefault(); handleAddUser(this);">
                    <div class="mb-4">
                        ...
                        <input type="text" id="user-name" name="user-name" required class="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                    </div>
                    <div class="mb-4">
                        <label for="user-role" class="block text-sm font-medium text-gray-700">Role</label>
                        <select id="user-role" name="user-role" required class="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" onchange="toggleStudentClassField()" oninput="toggleStudentClassField()">
                    ...
                    <div class="mb-4">
                        <label for="user-username" class="block text-sm font-medium text-gray-700" id="username-label">Username (ID / Admission No.)</label>
                        <input type="text" id="user-username" name="user-username" required class="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" placeholder="TID-001 or ADM-2023-123">
                    </div>
                    <div id="student-class-field" class="mb-4 hidden">
                        <label for="student-class" class="block text-sm font-medium text-gray-700">Student Class</label>
                        <select id="student-class" name="student-class" class="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                            <option value="" disabled selected>Select Class</option>
                            ${ALL_CLASSES.map(c => `<option value="${c}">${c}</option>`).join('')}
                        </select>
                    </div>
                    <div class="mb-6">
                        <label for="user-password" class="block text-sm font-medium text-gray-700">Temporary Password</label>
                        <input type="text" id="user-password" name="user-password" required value="123456" class="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                    </div>
    ...
    // Add event listener for role selection after DOM is ready
    setTimeout(() => {
        const roleSelect = document.getElementById('user-role');
        if (roleSelect) {
            // Remove any existing listeners first
            roleSelect.removeEventListener('change', toggleStudentClassField);
            // Add the event listener
            roleSelect.addEventListener('change', toggleStudentClassField);
            console.log('✅ Event listener added to role select');
...
window.switchTab = (tab) => {
    ...
    document.getElementById('add-users-section')?.classList.toggle('hidden', tab !== 'add-users');
    document.getElementById('users-section')?.classList.toggle('hidden', tab !== 'users');
    document.getElementById('notifications-section')?.classList.toggle('hidden', tab !== 'notifications');
    document.getElementById('marks-config-section')?.classList.toggle('hidden', tab !== 'marks-config');
...
window.toggleStudentClassField = () => {
    ...
    console.log('=== TOGGLE CLASS FIELD ===');
    ...
    if (roleSelect.value === 'student') {
        ...
        if (usernameLabel) {
            usernameLabel.textContent = 'Student ID / Admission No.';
        }
        console.log('✅ Class field shown for student');
        console.log('Class field classes:', classField.className);
    } else {
        ...
        if (usernameLabel) {
            usernameLabel.textContent = 'Teacher ID / Username';
        }
        console.log('❌ Class field hidden for teacher');
        console.log('Class field classes:', classField.className);
    }
    console.log('=== END TOGGLE ===');
};

// Debug function for add user form
window.debugAddUserForm = () => {
    console.log('=== ADD USER FORM DEBUG ===');
    ...
    console.log('Class field visibility:', {
        hidden: classField?.classList.contains('hidden'),
        classes: classField?.className
    });
    ...
    if (roleField) {
        ...
        if (nameField) nameField.value = 'Test Student';
        if (usernameField) usernameField.value = 'TEST001';
        if (classSelect) classSelect.value = '5th';
        
        console.log('Sample data filled. Current form state:');
        console.log({
            name: nameField?.value,
            role: roleField?.value,
            username: usernameField?.value,
...
window.testFormSubmission = () => {
    ...
    // Fill test data
    if (nameField) nameField.value = 'Test Student';
    if (usernameField) usernameField.value = 'TEST' + Date.now();
    
    if (roleField) {
        roleField.value = 'student';
        console.log('Set role to student, value is now:', roleField.value);
        
        // Trigger the change event
        roleField.dispatchEvent(new Event('change'));
        roleField.dispatchEvent(new Event('input'));
        
        // Wait a bit then set class
        setTimeout(() => {
            if (classField) {
                classField.value = '5th';
                console.log('Set class to 5th, value is now:', classField.value);
            }
            
            // Now test the form submission
            console.log('Testing form submission with filled data...');
            if (form) {
                handleAddUser(form);
            }
        }, 500);
...
window.handleAddUser = async (form) => {
    ...
    try {
        ...
        // Reset form
        form.reset();
        document.getElementById('student-class-field').classList.add('hidden');
...
const renderTeacherPanel = () => {
    ...
    panel.innerHTML = `
        <h2 class="text-2xl sm:text-3xl font-bold text-primary mb-4 sm:mb-6">Teacher Dashboard</h2>
        <p class="text-gray-600 mb-6">Welcome, ${currentUserData.name}</p>
        ...
        <!-- Teacher Tabs -->
        <div class="flex border-b border-gray-300 mb-6">
            <button data-tab-name="homework" onclick="switchTeacherTab('homework')" class="teacher-tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-white border-b-4 border-primary font-bold">
                <i data-lucide="clipboard-list" class="w-4 h-4 mr-2"></i> Homework Management
            </button>
            <button data-tab-name="notifications" onclick="switchTeacherTab('notifications')" class="teacher-tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="bell" class="w-4 h-4 mr-2"></i> Notifications ${getUnreadNotificationBadge('teachers')}
            </button>
            <button data-tab-name="marks" onclick="switchTeacherTab('marks')" class="teacher-tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="calculator" class="w-4 h-4 mr-2"></i> Marks Entry
            </button>
        </div>

        <!-- Homework Management Section -->
        <div id="homework-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            <h3 class="text-xl font-semibold text-gray-800 mb-6 border-b pb-3 flex items-center">
                ...
                <form id="homework-form" onsubmit="event.preventDefault(); handleAddHomework(this);">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <select id="homework-class" required class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" onchange="updateSubjectOptions()">
                                <option value="" disabled selected>Select Class</option>
                        ...
                        <div>
                            ...
                            <select id="homework-subject" required class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                                <option value="" disabled selected>Select Subject</option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="text" id="homework-title" required class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" placeholder="e.g., Chapter 5 Exercise Questions">
                    </div>
                    <div class="mb-4">
                        <label for="homework-description" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                        <textarea id="homework-description" required rows="3" class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" placeholder="Detailed homework instructions..."></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="homework-due-date" class="block text-sm font-medium text-gray-700 mb-2">Due Date</label>
                        <input type="date" id="homework-due-date" required class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                    </div>
                    <div class="mb-6">
                        <label for="homework-attachment" class="block text-sm font-medium text-gray-700 mb-2">Attachment (Optional)</label>
                        <input type="file" id="homework-attachment" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
...
window.switchTeacherTab = (tab) => {
    ...
    // Update button styling for teacher tabs
    document.querySelectorAll('.teacher-tab-button').forEach(btn => {
        if (btn.dataset.tabName === tab) {
            btn.classList.replace('bg-gray-200', 'bg-white');
            btn.classList.add('border-b-4', 'border-primary', 'font-bold');
            btn.classList.remove('text-gray-600', 'hover:bg-gray-100', 'border-b-2', 'border-gray-200', 'font-medium');
    ...
    // Update content visibility
    document.getElementById('homework-section')?.classList.toggle('hidden', tab !== 'homework');
    document.getElementById('teacher-notifications-section')?.classList.toggle('hidden', tab !== 'notifications');
    document.getElementById('marks-section')?.classList.toggle('hidden', tab !== 'marks');
    
    // If switching to marks tab, trigger form state update
    if (tab === 'marks') {
        console.log('Switched to marks tab, triggering form update');
        setTimeout(() => {
            updateMarksFormState();
        }, 100);
    }
...
const renderStudentPanel = () => {
    ...
    panel.innerHTML = `
        <h2 class="text-2xl sm:text-3xl font-bold text-primary mb-4 sm:mb-6">Student Dashboard</h2>
        <p class="text-gray-600 mb-6">Welcome, ${currentUserData.name} (Class: ${currentUserData.class})</p>
        ...
        <!-- Student Tabs -->
        <div class="flex border-b border-gray-300 mb-6">
            <button data-tab-name="homework" onclick="switchStudentTab('homework')" class="student-tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-white border-b-4 border-primary font-bold">
                <i data-lucide="book" class="w-4 h-4 mr-2"></i> My Homework
            </button>
            <button data-tab-name="notifications" onclick="switchStudentTab('notifications')" class="student-tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="bell" class="w-4 h-4 mr-2"></i> Notifications ${getUnreadNotificationBadge('students')}
            </button>
            <button data-tab-name="marksheet" onclick="switchStudentTab('marksheet')" class="student-tab-button flex items-center justify-center flex-1 py-3 px-4 rounded-t-lg text-sm font-medium transition duration-150 bg-gray-200 text-gray-600 hover:bg-gray-100 border-b-2 border-gray-200">
                <i data-lucide="file-text" class="w-4 h-4 mr-2"></i> My Marksheet
            </button>
        </div>

        <!-- Homework Section -->
        <div id="homework-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            <div class="homework-header-mobile flex justify-between items-center mb-6 border-b pb-3">
...
window.switchStudentTab = (tab) => {
    ...
    // Update button styling for student tabs
    ...
    // Update content visibility
    document.getElementById('homework-section')?.classList.toggle('hidden', tab !== 'homework');
    document.getElementById('student-notifications-section')?.classList.toggle('hidden', tab !== 'notifications');
    document.getElementById('marksheet-section')?.classList.toggle('hidden', tab !== 'marksheet');
