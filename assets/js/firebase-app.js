...
const SESSION_KEY = 'school_mgmt_session';
...
let principals = []; // Array of principal objects
let allotments = [];
let homeworks = [];
let marks = [];
...
const initializeAdminUser = async () => {
    ...
    const mockPrincipalId = 'PRI_12345';
    const adminDocRef = db.collection(USERS_LIST_COLLECTION_PATH).doc(mockAdminId);
    const principalDocRef = db.collection(USERS_LIST_COLLECTION_PATH).doc(mockPrincipalId);
    ...
    const principalData = {
        ...
        role: 'principal',
        username: 'principal', // LOGIN USERNAME
        loginPassword: 'principal', // LOGIN PASSWORD
        isOnline: false,
        lastLoginTime: null,
        lastLogoutTime: null
    };
    try {
        // Use set with merge to ensure documents are created if they don't exist
        await adminDocRef.set(adminData, { merge: true });
        console.log("Admin user initialized:", mockAdminId);
        
        await principalDocRef.set(principalData, { merge: true });
        console.log("Principal user initialized:", mockPrincipalId);
        
        // Verify the documents were created
        const adminCheck = await adminDocRef.get();
        const principalCheck = await principalDocRef.get();
        
        if (adminCheck.exists && principalCheck.exists) {
            console.log("✅ Both Admin and Principal users verified in Firestore");
        } else {
            console.error("❌ Failed to verify user creation in Firestore");
        }
        
        // Initialize default max marks configuration if not exists
        await initializeMaxMarksConfig();
    } catch (error) {
        console.error("Failed to initialize System Admin and Principal:", error);
        showNotificationModal("Database Error", "Failed to create initial admin and principal users. Check Firestore security rules.");
    }
};
...
const restoreSession = () => {
    ...
    if (session && userList[session.userId]) {
        // Verify user still exists in the system
        const user = userList[session.userId];
        currentUserId = user.uid;
        ...
        // Track login restoration and update lastLoginTime to prevent immediate cleanup
        trackUserLogin(user.uid);
        
        // Start keep-alive for restored session
        startKeepAlive();
        return true;
    }
    return false;
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Start automatic login activity cleanup after data is loaded
        setTimeout(() => {
            startLoginActivityCleanup();
...
// Track user login activity (set online status and update login time)
const trackUserLogin = async (userId) => {
    try {
        const userDocRef = db.collection(USERS_LIST_COLLECTION_PATH).doc(userId);
        
        // Check if document exists first
        const userDoc = await userDocRef.get();
        if (!userDoc.exists) {
            console.error(`User document ${userId} does not exist in Firestore`);
            return;
        }
        ...
        // Update local data
        if (userList[userId]) {
            Object.assign(userList[userId], updates);
        }
        
        console.log(`User ${userId} login tracked successfully`);
    } catch (error) {
...
const trackUserLogout = async (userId) => {
    try {
        ...
        if (userList[userId]) {
            userList[userId].isOnline = false;
            userList[userId].lastLogoutTime = new Date().toISOString();
        }
        
        console.log(`User ${userId} logout tracked`);
    } catch (error) {
...
window.showRoleSelector = async () => {
    // Track logout if user was logged in
    if (currentUserId) {
        await trackUserLogout(currentUserId);
    }
    
    // Stop keep-alive and clear session
    stopKeepAlive();
    clearSession();
    ...
    // First update content, then adjust header to prevent flicker
    const contentPanel = document.getElementById('content-panel');
    const header = document.getElementById('header');
    const userDropdown = document.querySelector('.user-dropdown');
    const marquee = document.getElementById('welcome-marquee');
    ...
    // Then adjust header and dropdown after content is ready
    setTimeout(() => {
        ...
        // Hide welcome marquee on login page
        if (marquee) {
    ...
    }, 10); // Very small delay to prevent flicker
};
...
window.handleLogin = async () => {
    ...
    if (user) {
        ...
        // Track login time and set online status
        await trackUserLogin(user.uid);

        // Start keep-alive for new login
        startKeepAlive();
...
const updateHeader = (user) => {
    ...
    // Show welcome marquee for authenticated users
    document.getElementById('welcome-marquee').classList.remove('hidden');
...
// --- PRINCIPAL PANEL FUNCTIONS ---
let activePrincipalTab = 'homework'; // 'homework', 'achiever', or 'notifications'

const renderPrincipalPanel = () => {
    const panel = document.getElementById('content-panel');
    panel.innerHTML = `
        <!-- Principal Panel Tabs -->
        <div class="principal-tabs-container">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto mb-6">
                <button data-tab-name="homework" onclick="switchPrincipalTab('homework')" class="principal-tab-card bg-blue-500 hover:bg-blue-600 text-white p-6 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105">
                    <div class="flex flex-col items-center">
                        <i data-lucide="book-open" class="w-8 h-8 mb-2"></i>
                        <h3 class="text-lg font-semibold mb-1">Homework Overview</h3>
                        <p class="text-blue-100 text-sm">Monitor class assignments</p>
                    </div>
                </button>
                
                <button data-tab-name="achiever" onclick="switchPrincipalTab('achiever')" class="principal-tab-card bg-yellow-500 hover:bg-yellow-600 text-white p-6 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105">
                    <div class="flex flex-col items-center">
                        <i data-lucide="trophy" class="w-8 h-8 mb-2"></i>
                        <h3 class="text-lg font-semibold mb-1">Class Toppers</h3>
                        <p class="text-yellow-100 text-sm">View academic achievers</p>
                    </div>
                </button>
                
                <button data-tab-name="notifications" onclick="switchPrincipalTab('notifications')" class="principal-tab-card bg-purple-500 hover:bg-purple-600 text-white p-6 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105">
                    <div class="flex flex-col items-center">
                        <i data-lucide="bell" class="w-8 h-8 mb-2"></i>
                        <h3 class="text-lg font-semibold mb-1">Notifications</h3>
                        <p class="text-purple-100 text-sm">Important announcements</p>
                    </div>
                </button>
            </div>
        </div>
        ...
        <div id="principal-homework-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 hidden">
            <div class="flex items-center justify-between mb-6 border-b pb-3">
                <h3 class="text-xl font-semibold text-gray-800 flex items-center">
                    <i data-lucide="book-open" class="w-5 h-5 mr-2"></i>
                    Class-wise Homework Overview
                </h3>
                <button onclick="showPrincipalTabs()" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition duration-150 flex items-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label for="homework-class-filter" class="block text-sm font-medium text-gray-700 mb-2">Select Class</label>
                    <select id="homework-class-filter" class="w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:border-purple-600 focus:ring focus:ring-purple-600 focus:ring-opacity-50 transition duration-150" onchange="filterPrincipalHomework()">
                        <option value="">All Classes</option>
                        ${ALL_CLASSES.map(classId => `<option value="${classId}">${classId} Class</option>`).join('')}
                    </select>
                </div>
                <div>
                    <label for="homework-date-filter" class="block text-sm font-medium text-gray-700 mb-2">Select Date</label>
                    <input type="date" id="homework-date-filter" class="w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:border-purple-600 focus:ring focus:ring-purple-600 focus:ring-opacity-50 transition duration-150" onchange="filterPrincipalHomework()">
                </div>
            </div>
            
            <!-- Homework List -->
            <div id="principal-homework-list" class="space-y-4">
                <!-- Homework items will be populated here -->
            </div>
        </div>

        <!-- Class Achievers Section -->
        <div id="principal-achiever-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 hidden">
            <div class="flex items-center justify-between mb-6 border-b pb-3">
                <h3 class="text-xl font-semibold text-gray-800 flex items-center">
                    <i data-lucide="trophy" class="w-5 h-5 mr-2"></i>
                    Class Toppers & Achievers
                </h3>
                <button onclick="showPrincipalTabs()" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition duration-150 flex items-center">
                    <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i>
                    Back to Dashboard
            ...
            <div id="principal-achiever-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Achiever cards will be populated here -->
            </div>
        </div>

        <!-- Notifications Section -->
        <div id="principal-notifications-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 hidden">
            <div class="flex items-center justify-between mb-6 border-b pb-3">
                <h3 class="text-xl font-semibold text-gray-800 flex items-center">
            ...
            <div id="principal-notifications-list" class="space-y-4">
                <!-- Notifications will be populated here -->
            </div>
        </div>
    `;
    
    // Initialize - show only tabs, hide all content sections
    setTimeout(() => {
        showPrincipalTabs();
    }, 100);
...
// Show principal tabs (hide content sections)
window.showPrincipalTabs = () => {
    // Hide all content sections
    document.getElementById('principal-homework-section').classList.add('hidden');
    document.getElementById('principal-achiever-section').classList.add('hidden');
    document.getElementById('principal-notifications-section').classList.add('hidden');
    
    // Show tabs container
    document.querySelector('.principal-tabs-container').classList.remove('hidden');
    
    // Reset all card styling
    document.querySelectorAll('.principal-tab-card').forEach(btn => {
        btn.classList.remove('ring-4', 'ring-white', 'ring-opacity-50', 'transform', 'scale-105');
        btn.style.boxShadow = '0 4px 6px rgba(0, 0, 0, 0.1)';
    });
};

// Switch between principal tabs
window.switchPrincipalTab = (tab) => {
    activePrincipalTab = tab;
    
    // Hide tabs container
    document.querySelector('.principal-tabs-container').classList.add('hidden');
    
    // Show/hide sections
    document.getElementById('principal-homework-section').classList.toggle('hidden', tab !== 'homework');
    document.getElementById('principal-achiever-section').classList.toggle('hidden', tab !== 'achiever');
    document.getElementById('principal-notifications-section').classList.toggle('hidden', tab !== 'notifications');
    
    // Load content based on tab
    if (tab === 'homework') {
        renderPrincipalHomework();
    } else if (tab === 'achiever') {
        renderPrincipalAchievers();
    } else if (tab === 'notifications') {
        renderPrincipalNotifications();
    }
...
// Principal homework rendering function
const renderPrincipalHomework = () => {
    const homeworkList = document.getElementById('principal-homework-list');
    if (!homeworkList) return;
    
    const classFilter = document.getElementById('homework-class-filter')?.value || '';
    const dateFilter = document.getElementById('homework-date-filter')?.value || '';
    
    // Filter homework based on class and date
    let filteredHomework = homeworks.filter(hw => {
        const matchesClass = !classFilter || hw.class === classFilter;
        const matchesDate = !dateFilter || hw.assignedDate === dateFilter;
        return matchesClass && matchesDate;
    });
    
    // Group homework by class
    const homeworkByClass = filteredHomework.reduce((acc, hw) => {
        if (!acc[hw.class]) acc[hw.class] = [];
        acc[hw.class].push(hw);
        return acc;
    }, {});
    
    if (Object.keys(homeworkByClass).length === 0) {
        homeworkList.innerHTML = `
            <div class="text-center py-8 text-gray-500">
                <i data-lucide="book-open" class="w-12 h-12 mx-auto mb-4 text-gray-300"></i>
                <p class="text-lg font-medium">No homework found</p>
    ...
    homeworkList.innerHTML = Object.entries(homeworkByClass).map(([className, classHomework]) => `
        <div class="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 rounded-lg border border-purple-200">
            <h4 class="text-lg font-semibold text-purple-800 mb-4 flex items-center">
                ...
                Class ${className} - ${classHomework.length} Assignment${classHomework.length > 1 ? 's' : ''}
            </h4>
            <div class="space-y-3">
                ${classHomework.map(hw => {
                    const teacher = teachers.find(t => t.uid === hw.teacherId);
                    ...
                    return `
                        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <h5 class="font-semibold text-gray-800">${hw.subject}</h5>
                                    <p class="text-sm text-gray-600">by ${teacherName}</p>
                                </div>
                            ...
                            <p class="text-gray-700 text-sm mb-2">${hw.description}</p>
                            <div class="flex items-center justify-between">
                                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-800">
                                    ${hw.priority || 'Normal'} Priority
                                </span>
...
// Filter principal homework
window.filterPrincipalHomework = () => {
    renderPrincipalHomework();
};

// Principal achievers rendering function
const renderPrincipalAchievers = () => {
    const achieverList = document.getElementById('principal-achiever-list');
    if (!achieverList) return;
    
    // Calculate class-wise toppers with overall percentage (not term-wise)
    const classToppers = {};
    ...
    ALL_CLASSES.forEach(className => {
        ...
        const studentAverages = classStudents.map(student => {
            // Get all marks for this student across all subjects and terms
            const studentMarks = marks.filter(m => m.studentId === student.uid);
            if (studentMarks.length === 0) return { student, average: 0, totalMarks: 0, totalPossible: 0 };
            
            // Calculate overall percentage across all subjects and terms (same as student marksheet)
            const totalMarks = studentMarks.reduce((sum, mark) => sum + (mark.marks || 0), 0);
            const totalPossible = studentMarks.reduce((sum, mark) => sum + (mark.maxMarks || 0), 0);
            const average = totalPossible > 0 ? Math.round((totalMarks / totalPossible) * 100) : 0;
            const grade = calculateGrade(average);
            
            return { student, average, totalMarks, totalPossible, grade };
        }).filter(s => s.totalPossible > 0);
        
        // Sort by average and get top 3 for each class
        studentAverages.sort((a, b) => b.average - a.average);
        classToppers[className] = studentAverages.slice(0, 3);
    });
    
    const hasToppers = Object.values(classToppers).some(toppers => toppers.length > 0);
    
    if (!hasToppers) {
        achieverList.innerHTML = `
            <div class="col-span-full text-center py-8 text-gray-500">
                <i data-lucide="trophy" class="w-12 h-12 mx-auto mb-4 text-gray-300"></i>
                <p class="text-lg font-medium">No achievers data available</p>
                <p class="text-sm">Marks need to be entered to calculate class toppers</p>
            </div>
        `;
        if (typeof lucide !== 'undefined') lucide.createIcons();
        return;
    ...
    // Create HTML for class-wise toppers only (with overall percentage)
    achieverList.innerHTML = Object.entries(classToppers)
        .filter(([className, toppers]) => toppers.length > 0)
        .map(([className, toppers]) => `
            <div class="bg-gradient-to-br from-yellow-50 to-orange-50 p-6 rounded-xl shadow-lg border border-yellow-200">
                <h4 class="text-lg font-bold text-orange-800 mb-4 flex items-center">
                    ...
                    Class ${className} Toppers
                    <span class="ml-2 text-sm font-normal text-gray-600">(Overall Performance)</span>
                </h4>
                <div class="space-y-3">
                    ${toppers.map((topper, index) => {
                        const position = index + 1;
                        const medal = position === 1 ? '🥇' : position === 2 ? '🥈' : '🥉';
                        const bgColor = position === 1 ? 'bg-yellow-100 border-yellow-300' : position === 2 ? 'bg-gray-100 border-gray-300' : 'bg-orange-100 border-orange-300';
                        
                        return `
                            <div class="${bgColor} p-4 rounded-lg border-2">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <span class="text-xl mr-3 font-bold">${medal}</span>
                                        <div>
                                            <div class="font-semibold text-gray-800">${topper.student.name}</div>
                                            <div class="text-sm text-gray-600">${topper.student.username}</div>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-xl ${position === 1 ? 'text-yellow-600' : position === 2 ? 'text-gray-600' : 'text-orange-600'}">
                                            ${topper.average}%
                                        </div>
                                    </div>
                                </div>
...
// Principal notifications rendering function
const renderPrincipalNotifications = () => {
    const notificationsList = document.getElementById('principal-notifications-list');
    if (!notificationsList) return;
    
    // Filter notifications for principals (all notifications or those targeted to principals)
    const relevantNotifications = notifications.filter(n => 
        n.target === 'all' || n.target === 'principals'
    ...
    if (relevantNotifications.length === 0) {
        notificationsList.innerHTML = `
            <div class="text-center py-8 text-gray-500">
                ...
                <p class="text-sm">All notifications will appear here</p>
            </div>
        `;
        if (typeof lucide !== 'undefined') lucide.createIcons();
        return;
    ...
    notificationsList.innerHTML = relevantNotifications.map(notification => {
        const createdDate = notification.createdAt ? new Date(notification.createdAt).toLocaleDateString() : 'Unknown Date';
        const createdTime = notification.createdAt ? new Date(notification.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'Unknown Time';
        ...
        const message = notification.content || notification.message || 'No Content';
        ...
        return `
            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                <div class="flex justify-between items-start mb-2">
                    <h5 class="font-semibold text-gray-800">${title}</h5>
                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full ${priorityColor}">
                        ${priority.charAt(0).toUpperCase() + priority.slice(1)}
                    </span>
                </div>
                <p class="text-gray-700 text-sm mb-3">${message}</p>
                <div class="flex justify-between items-center text-xs text-gray-500">
                    <span>Target: ${target === 'all' ? 'All Users' : target.charAt(0).toUpperCase() + target.slice(1)}</span>
...
const renderAdminPanel = () => {
    ...
    panel.innerHTML = `
        ...
        <div class="admin-tabs-container">
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 max-w-6xl mx-auto">
                ...
                <button data-tab-name="login-activity" onclick="openAdminSection('login-activity')" class="admin-tab-card bg-teal-500 hover:bg-teal-600 text-white p-4 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105">
                    <div class="flex flex-col items-center">
                        <i data-lucide="activity" class="w-8 h-8 mb-2"></i>
                        <h3 class="text-lg font-semibold mb-1">Login Activity</h3>
                        <p class="text-teal-100 text-sm">Track user logins & status</p>
                    </div>
                </button>
            </div>
        </div>
                ...
                <form id="add-user-form" onsubmit="event.preventDefault(); handleAddUser(this);">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <button type="button" id="role-teacher" onclick="setUserRole('teacher')" class="role-option-btn flex flex-col items-center p-4 border-2 border-gray-300 rounded-lg hover:border-blue-500 transition-colors">
                                <i data-lucide="graduation-cap" class="w-6 h-6 mb-2 text-gray-600"></i>
                                <span class="text-sm font-medium text-gray-700">Teacher</span>
                            </button>
                            <button type="button" id="role-student" onclick="setUserRole('student')" class="role-option-btn flex flex-col items-center p-4 border-2 border-blue-500 bg-blue-50 rounded-lg transition-colors">
                                <i data-lucide="user" class="w-6 h-6 mb-2 text-blue-600"></i>
                                <span class="text-sm font-medium text-blue-700">Student</span>
                            </button>
                            <button type="button" id="role-principal" onclick="setUserRole('principal')" class="role-option-btn flex flex-col items-center p-4 border-2 border-gray-300 rounded-lg hover:border-purple-500 transition-colors">
                                <i data-lucide="crown" class="w-6 h-6 mb-2 text-gray-600"></i>
        ...
        <div id="login-activity-section" class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 hidden">
            <h3 class="text-xl font-semibold text-gray-800 mb-6 border-b pb-3 flex items-center">
                <i data-lucide="activity" class="w-5 h-5 mr-2"></i>
                User Login Activity & Status
            </h3>
            ...
            <div class="bg-gradient-to-r from-teal-50 to-cyan-50 p-6 rounded-lg border border-teal-200">
                ...
                <div class="mb-8">
                    <h4 class="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                        ...
                        <span id="online-count" class="ml-2 bg-green-100 text-green-800 text-sm font-medium px-2.5 py-0.5 rounded-full">0</span>
                    </h4>
                    <div id="online-users-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody id="login-activity-table" class="bg-white divide-y divide-gray-200">
                                    <!-- Login activity rows will be populated here -->
                                </tbody>
                            </table>
                        </div>
                    </div>
                ...
                <div class="flex justify-center space-x-4">
                    <button onclick="refreshLoginActivity()" class="bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2 px-6 rounded-lg transition duration-150 flex items-center">
                        <i data-lucide="refresh-cw" class="w-4 h-4 mr-2"></i>
                        Refresh Activity
                    </button>
                    <button onclick="cleanupOldLoginActivity()" class="bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2 px-6 rounded-lg transition duration-150 flex items-center">
                        <i data-lucide="clock" class="w-4 h-4 mr-2"></i>
                        Clear 30min+ Activity
                    </button>
                    <button onclick="forceCleanupAllOldActivity()" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-lg transition duration-150 flex items-center">
    ...
    setTimeout(() => {
        // Set default to student mode
        setUserRole('student');
        console.log('✅ Role selector initialized to student mode');
    }, 200);
...
window.openAdminSection = (section) => {
    ...
    document.getElementById('login-activity-section')?.classList.add('hidden');
    ...
    // Initialize login activity if it's the login-activity section
    if (section === 'login-activity') {
        setTimeout(() => {
            renderLoginActivity();
        }, 100);
    }
...
window.refreshLoginActivity = () => {
    renderLoginActivity();
};

// Comprehensive cleanup function for old login activities
window.cleanupOldLoginActivity = async () => {
    try {
        const now = new Date();
        ...
        console.log(`   - 30min threshold: ${thirtyMinutesAgo.toISOString()}`);
        console.log(`   - 1hr threshold: ${oneHourAgo.toISOString()}`);
        console.log(`   - 24hr threshold: ${twentyFourHoursAgo.toISOString()}`);
        
        // Get all users and check for old login activities
        const allUsers = [...teachers, ...students, ...principals];
        let onlineToOfflineCount = 0; // Users marked offline (30min rule)
        let oldActivityCleanupCount = 0; // Users with old login records cleaned
        let skippedCount = 0;
        let veryOldActivityCount = 0; // Users with 24hr+ old activity
        ...
        for (const user of allUsers) {
            ...
            if (user.lastLoginTime) {
                ...
                // Skip cleanup for currently active user (the one using the system right now)
                if (currentUserId && user.uid === currentUserId) {
                    console.log(`⏭️ Skipping cleanup for current active user: ${user.name}`);
                    skippedCount++;
                    continue;
                }
                
                // TIER 1: Mark online users offline after 30 minutes of inactivity AND clear recent activity
                if (user.isOnline && lastLoginDate < thirtyMinutesAgo) {
                    updates.isOnline = false;
                    updates.lastLogoutTime = now.toISOString();
                    // Clear recent login activity by resetting lastLoginTime to null
                    updates.lastLoginTime = null;
                    needsUpdate = true;
                    onlineToOfflineCount++;
                    console.log(`⏰ [30min] Marking user ${user.name} (${user.username}) as offline and clearing recent activity - was last active: ${lastLoginDate.toLocaleString()}`);
                }
                
                // TIER 2: Clean up login records for users inactive 1-24 hours
                else if (!user.isOnline && lastLoginDate < oneHourAgo && lastLoginDate >= twentyFourHoursAgo) {
                    // Clear activity records for users inactive between 1-24 hours
                    updates.lastLoginTime = null;
                    needsUpdate = true;
                    oldActivityCleanupCount++;
                    console.log(`🕐 [1hr-24hr] Clearing login activity records for ${user.name} - inactive since: ${lastLoginDate.toLocaleString()}`);
                }
                
                // TIER 3: Archive very old login activities (24+ hours)
                else if (lastLoginDate < twentyFourHoursAgo) {
                    if (user.isOnline) {
                        // Force offline users who somehow stayed online for 24+ hours
                        updates.isOnline = false;
                        updates.lastLogoutTime = now.toISOString();
                        needsUpdate = true;
                    }
                    
                    // Archive old login data and clear all activity records
                    updates.lastLoginTime = null;
                    needsUpdate = true;
                    veryOldActivityCount++;
                    console.log(`📦 [24hr+] Archiving and clearing very old activity for ${user.name} - last active: ${lastLoginDate.toLocaleString()}`);
                }
                
                // Active users (within 30 minutes)
                else if (user.isOnline && lastLoginDate >= thirtyMinutesAgo) {
                    console.log(`✅ User ${user.name} still active - last login: ${lastLoginDate.toLocaleString()}`);
                }
            }
            
            // Handle users with no lastLoginTime
            else if (user.isOnline) {
                console.log(`⚠️ User ${user.name} marked online but no lastLoginTime - marking offline`);
                updates.isOnline = false;
                updates.lastLogoutTime = now.toISOString();
                needsUpdate = true;
                onlineToOfflineCount++;
            ...
            if (needsUpdate) {
                try {
                    const userDocRef = db.collection(USERS_LIST_COLLECTION_PATH).doc(user.uid);
                    await userDocRef.update(updates);
                    
                    // Update local data
                    if (userList[user.uid]) {
                        Object.assign(userList[user.uid], updates);
                    }
                    
                    // Update in teachers, students, or principals array
                    if (user.role === 'teacher') {
                        const teacherIndex = teachers.findIndex(t => t.uid === user.uid);
                        if (teacherIndex !== -1) {
                            Object.assign(teachers[teacherIndex], updates);
                        }
                    } else if (user.role === 'student') {
                        const studentIndex = students.findIndex(s => s.uid === user.uid);
                        if (studentIndex !== -1) {
                            Object.assign(students[studentIndex], updates);
                        }
                    } else if (user.role === 'principal') {
                        const principalIndex = principals.findIndex(p => p.uid === user.uid);
                        if (principalIndex !== -1) {
                            Object.assign(principals[principalIndex], updates);
                        }
                    }
                } catch (error) {
        ...
        console.log(`   - ${onlineToOfflineCount} users marked offline and recent activity cleared (30min+ inactive)`);
        console.log(`   - ${oldActivityCleanupCount} users with login activity records cleared (1hr-24hr)`);
        console.log(`   - ${veryOldActivityCount} users with very old activity archived and cleared (24hr+)`);
        console.log(`   - ${skippedCount} active users skipped`);
        
        if (onlineToOfflineCount > 0 || oldActivityCleanupCount > 0 || veryOldActivityCount > 0) {
            // Refresh the login activity display if it's currently visible
            const loginActivitySection = document.getElementById('login-activity-section');
            if (loginActivitySection && !loginActivitySection.classList.contains('hidden')) {
                renderLoginActivity();
                
                // Show notification to admin if they're viewing the login activity section
                if (currentUserData && currentUserData.role === 'admin') {
                    const totalCleaned = onlineToOfflineCount + oldActivityCleanupCount + veryOldActivityCount;
                    console.log(`📢 Admin notification: ${totalCleaned} user login activities cleaned up automatically`);
                }
            }
        }
...
// Force cleanup function for aggressive cleanup of 30min+ old activities
window.forceCleanupAllOldActivity = async () => {
    try {
        const now = new Date();
        const thirtyMinutesAgo = new Date(now.getTime() - (30 * 60 * 1000)); // 30 minutes ago
        
        console.log(`🗑️ Starting FORCE cleanup: removing all activity older than 30 minutes (${thirtyMinutesAgo.toISOString()})`);
        
        // Get all users and check for old login activities
        const allUsers = [...teachers, ...students, ...principals];
        let forcedOfflineCount = 0;
        let activityClearedCount = 0;
        let skippedCount = 0;
        ...
        for (const user of allUsers) {
            ...
            // Skip cleanup for currently active user
            if (currentUserId && user.uid === currentUserId) {
                console.log(`⏭️ Skipping force cleanup for current active user: ${user.name}`);
                skippedCount++;
                continue;
            }
            ...
            if (user.lastLoginTime) {
                ...
                // Force offline any user with activity older than 30 minutes
                if (lastLoginDate < thirtyMinutesAgo) {
                    if (user.isOnline) {
                        updates.isOnline = false;
                        updates.lastLogoutTime = now.toISOString();
                        forcedOfflineCount++;
                        console.log(`🔴 [FORCE] Marking user ${user.name} offline - last active: ${lastLoginDate.toLocaleString()}`);
                    }
                    
                    // Clear login activity records completely
                    updates.lastLoginTime = null;
                    activityClearedCount++;
                    console.log(`🧹 [FORCE] Cleared all login activity for ${user.name} - was last active: ${lastLoginDate.toLocaleString()}`);
                    
                    needsUpdate = true;
                }
            }
            
            // Handle users with no lastLoginTime but marked online
            else if (user.isOnline) {
                console.log(`⚠️ [FORCE] User ${user.name} marked online but no lastLoginTime - forcing offline`);
                updates.isOnline = false;
                updates.lastLogoutTime = now.toISOString();
                needsUpdate = true;
                forcedOfflineCount++;
            ...
            if (needsUpdate) {
                try {
                    ...
                    console.error(`Error in force cleanup for user ${user.uid}:`, error);
                }
            }
        }
        ...
        console.log(`   - ${forcedOfflineCount} users forced offline (30min+ inactive)`);
        console.log(`   - ${activityClearedCount} user login activities completely cleared`);
        console.log(`   - ${skippedCount} active users skipped`);
        
        // Show notification to admin
        showNotificationModal('Force Cleanup Complete', 
            `Cleared ${activityClearedCount} user login activities older than 30 minutes. ${forcedOfflineCount} users marked offline.`);
        
        // Refresh the login activity display
        const loginActivitySection = document.getElementById('login-activity-section');
        if (loginActivitySection && !loginActivitySection.classList.contains('hidden')) {
            renderLoginActivity();
        }
        ...
        showNotificationModal('Cleanup Error', 'An error occurred during force cleanup. Check console for details.');
    }
};

// Schedule automatic cleanup every hour
let cleanupInterval;
const startLoginActivityCleanup = () => {
    ...
    window.cleanupOldLoginActivity();
    
    // Schedule cleanup every 15 minutes (900000 ms) for more responsive cleanup
    cleanupInterval = setInterval(window.cleanupOldLoginActivity, 900000);
    console.log('🕐 Comprehensive login activity cleanup scheduled every 15 minutes (30min/1hr/24hr tiers)');
};

// Stop the cleanup interval
const stopLoginActivityCleanup = () => {
    if (cleanupInterval) {
        clearInterval(cleanupInterval);
        cleanupInterval = null;
        console.log('🛑 Login activity cleanup stopped');
    }
};

// Keep current user's session active by updating lastLoginTime periodically
...
const startKeepAlive = () => {
    ...
    // Update current user's lastLoginTime every 10 minutes to keep them active
    keepAliveInterval = setInterval(async () => {
        if (currentUserId && currentUserData) {
            try {
                const now = new Date().toISOString();
                const userDocRef = db.collection(USERS_LIST_COLLECTION_PATH).doc(currentUserId);
                ...
                if (userList[currentUserId]) {
                    userList[currentUserId].lastLoginTime = now;
                    userList[currentUserId].isOnline = true;
                }
                
                console.log('💓 Keep-alive: Updated activity timestamp for current user');
            } catch (error) {
                console.error('Error in keep-alive update:', error);
            }
        }
    }, 600000); // Every 10 minutes (600000 ms)
    
    console.log('💓 Keep-alive started: Will update user activity every 10 minutes');
};
...
const renderLoginActivity = () => {
    ...
    const loginActivityTable = document.getElementById('login-activity-table');
    const onlineCount = document.getElementById('online-count');
    
    if (!onlineUsersList || !loginActivityTable || !onlineCount) {
        return;
    }
    ...
    // Render online users
    onlineUsersList.innerHTML = onlineUsers.length > 0 ? 
        ...
        `).join('') :
        '<div class="col-span-full text-center py-8 text-gray-500">No users currently online</div>';
    
    // Sort all users by last login time (most recent first)
    const sortedUsers = allUsers
        .filter(user => user.lastLoginTime)
        .filter(user => {
            const date = new Date(user.lastLoginTime);
            return !isNaN(date.getTime());
        })
        .sort((a, b) => new Date(b.lastLoginTime) - new Date(a.lastLoginTime))
        .slice(0, 20); // Show last 20 login activities
    
    // Render login activity table
    loginActivityTable.innerHTML = sortedUsers.length > 0 ?
        sortedUsers.map(user => {
            const lastLogin = user.lastLoginTime ? new Date(user.lastLoginTime) : null;
            const isValidDate = lastLogin && !isNaN(lastLogin.getTime());
            ...
            return `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="w-2 h-2 ${user.isOnline ? 'bg-green-500' : 'bg-gray-400'} rounded-full mr-2"></div>
                            <span class="text-sm ${user.isOnline ? 'text-green-600' : 'text-gray-500'} font-medium">
                                ${user.isOnline ? 'Online' : 'Offline'}
                            </span>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        ${isValidDate ? formatDateTime(lastLogin) : 'Never'}
                    </td>
                </tr>
            `;
        }).join('') :
        '<tr><td colspan="4" class="px-6 py-8 text-center text-gray-500">No login activity found</td></tr>';
...
window.setUserRole = (role) => {
    ...
    // Only proceed if we're in the add-users section and elements exist
    if (!roleInput || !document.getElementById('add-users-section')) return;
    ...
    document.querySelectorAll('.role-option-btn').forEach(btn => {
        if (btn) {
            ...
            if (text) {
                text.classList.remove('text-blue-700', 'text-green-700', 'text-purple-700');
    ...
    if (role === 'student') {
        ...
        if (usernameLabel) usernameLabel.textContent = 'Principal ID';
        if (usernameInput) usernameInput.placeholder = 'PRI-001';
        if (submitButtonText) submitButtonText.textContent = 'Add Principal to System';
    }
};

// Legacy function for compatibility
window.toggleStudentClassField = () => {
    // This function is now handled by setUserRole
    setUserRole('student');
};
...
const renderUserTables = (filteredTeachers, filteredStudents) => {
    ...
    // Combine teachers and principals for the teachers tab
    const teachersAndPrincipals = [...filteredTeachers, ...principals];
    
    if (teacherContainer) {
        teacherContainer.innerHTML = generateUserTableHTML(teachersAndPrincipals);
    }
    if (studentContainer) {
        studentContainer.innerHTML = generateUserTableHTML(filteredStudents);
    }

    // Update counts in buttons (teachers + principals)
    ...
    if (teacherButton) {
        const totalTeachersAndPrincipals = teachers.length + principals.length;
        teacherButton.innerHTML = `<i data-lucide="graduation-cap" class="w-4 h-4 mr-2"></i> Teachers (${totalTeachersAndPrincipals})`;
    }
    if (studentButton) {
...
window.filterUsers = () => {
    ...
    const filteredPrincipals = principals.filter(p => 
        p.username.toLowerCase().includes(searchTerm) || p.name.toLowerCase().includes(searchTerm)
    );
    ...
    // Combine teachers and principals for the teachers tab
    const teachersAndPrincipals = [...filteredTeachers, ...filteredPrincipals];
    
    if (teacherContainer) teacherContainer.innerHTML = generateUserTableHTML(teachersAndPrincipals);
...
window.handleEditUser = async (uid, form) => {
    ...
    if (role === 'student') {
        ...
        // Remove class field if changing from student to teacher/principal
        userData.class = null;
    }
...
window.handleImportStudentsFromExcel = async () => {
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // Convert values to strings and handle null/undefined values
            const name = row.name ? String(row.name).trim() : '';
            const username = row.username ? String(row.username).trim() : '';
            const className = row.class ? String(row.class).trim() : '';
            const loginPassword = row.loginPassword ? String(row.loginPassword).trim() : '123456';
            
            // Validate required fields
            if (!name || !username || !className) {
                errors.push(`Row ${rowNum}: Missing required fields (name: "${name}", username: "${username}", class: "${className}")`);
                continue;
            }
            
            // Validate class
            if (!ALL_CLASSES.includes(className)) {
                errors.push(`Row ${rowNum}: Invalid class "${className}". Must be one of: ${ALL_CLASSES.join(', ')}`);
                continue;
            }
            
            // Check for duplicate username in existing users
            const existingUser = Object.values(userList).find(u => u.username === username);
            if (existingUser) {
            ...
            const duplicateInBatch = validStudents.find(s => s.username === username);
            if (duplicateInBatch) {
                errors.push(`Row ${rowNum}: Duplicate username "${username}" in file`);
                continue;
            }
        ...
        if (validStudents.length === 0) {
            ...
            const errorMessage = `Found ${errorCount} error${errorCount > 1 ? 's' : ''} in the Excel file:\n\n${errors.slice(0, 10).join('\n')}${errorCount > 10 ? `\n\n...and ${errorCount - 10} more errors` : ''}`;
            showNotificationModal('Import Failed - No Valid Students', errorMessage);
            return;
        }
        ...
        .map(homework => `
            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                ${homework.attachmentUrl ? `
                    <div class="mb-2">
                        <button onclick="viewAttachment('${homework.id}')" class="text-blue-600 hover:text-blue-800 text-sm flex items-center cursor-pointer">
...
window.viewAttachment = (homeworkId) => {
    ...
    try {
        const attachmentUrl = homework.attachmentUrl;
        const attachmentName = homework.attachmentName || 'attachment';
        const attachmentType = homework.attachmentType || '';
        
        // Create a new window with proper HTML content for viewing the attachment
            ...
            htmlContent = `
                ...
                <body>
                    <div class="container">
                        ...
                        <br>
                        <a href="${attachmentUrl}" download="${attachmentName}" class="download-btn">📥 Download Image</a>
                    </div>
                </body>
                </html>
            `;
            ...
            htmlContent = `
                ...
                <body>
                    <div class="header">
                        ...
                        <a href="${attachmentUrl}" download="${attachmentName}" class="download-btn">📥 Download PDF</a>
                    </div>
                    <iframe src="${attachmentUrl}" type="application/pdf">
                        <div class="fallback">
                            <p>Your browser doesn't support PDF viewing.</p>
                            <a href="${attachmentUrl}" download="${attachmentName}" style="color: #1976d2;">Click here to download the PDF</a>
                        </div>
                    </iframe>
                </body>
                </html>
            ...
            htmlContent = `
                ...
                <body>
                    <div class="container">
                        ...
                        <h2>${attachmentName}</h2>
                        <div class="file-info">
                            <p>This file type cannot be previewed in the browser.</p>
                            <p>Click the download button below to view the file.</p>
                        </div>
                        <a href="${attachmentUrl}" download="${attachmentName}" class="download-btn">📥 Download File</a>
                    </div>
                </body>
                </html>
            `;
                ...
                <table class="w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ${subjects.map((subject, index) => {
                            ...
                            // Calculate subject average (only for marks >= 40%)
                            const validMarks = Object.values(subjectMarks).filter(m => m.percentage !== undefined && m.percentage >= 40);
                            const avgPercentage = validMarks.length > 0 
                                ? Math.round(validMarks.reduce((sum, m) => sum + m.percentage, 0) / validMarks.length)
                                : 0;
                            const avgGrade = validMarks.length > 0 ? calculateGrade(avgPercentage) : '-';
                            
                            // Check if any marks are below 40%
                            const hasFailingMarks = Object.values(subjectMarks).some(m => m.percentage < 40);
                                    ...
                                    ${EXAM_TYPES.map(examType => {
                                        ...
                                        return `
                                            <td class="px-3 py-4 whitespace-nowrap text-center">
                                                ...
                                                ` : `
                                                    <div class="text-xs text-red-500">Below 40%</div>
                                                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">-</span>
                                                `}
                                            </td>
                                        `;
                                    }).join('')}
                                    <td class="px-4 py-4 whitespace-nowrap text-center">
                                        ${hasMarks && !hasFailingMarks && validMarks.length > 0 ? `
...
const renderPerformanceSummary = () => {
    ...
    // Filter marks >= 40% for calculations
    const passingMarks = studentMarks.filter(m => m.percentage >= 40);
    const hasFailingMarks = studentMarks.some(m => m.percentage < 40);
    
    // Calculate overall statistics (only for passing marks)
    const totalMarks = passingMarks.reduce((sum, m) => sum + m.marks, 0);
    const totalMaxMarks = passingMarks.reduce((sum, m) => sum + m.maxMarks, 0);
    const overallPercentage = passingMarks.length > 0 ? Math.round((totalMarks / totalMaxMarks) * 100) : 0;
    const overallGrade = passingMarks.length > 0 && !hasFailingMarks ? calculateGrade(overallPercentage) : '-';
    
    // Grade distribution (only for passing marks)
    const gradeCount = passingMarks.reduce((acc, m) => {
        acc[m.grade] = (acc[m.grade] || 0) + 1;
        return acc;
    }, {});
    
    // Subject performance (only for passing marks)
    const subjectPerformance = {};
    passingMarks.forEach(mark => {
    ...
    return `
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="text-center">
                ${hasFailingMarks ? `
                    <div class="text-2xl font-bold text-red-500 mb-2">Below 40%</div>
                    <div class="text-sm text-gray-600 mb-2">Overall Performance</div>
                    <span class="px-3 py-1 text-sm font-semibold rounded-full bg-red-100 text-red-800">- Grade</span>
                ` : `
            ...
            <!-- Best Subject -->
            <div class="text-center">
                ${bestSubject.subject && !hasFailingMarks ? `
            ...
            <div class="flex flex-wrap gap-2">
                ${hasFailingMarks ? `
                    <span class="px-3 py-1 text-sm font-semibold rounded-full bg-red-100 text-red-800">
                        Some marks below 40% - Grades not displayed
                    </span>
                ` : Object.entries(gradeCount).length > 0 ? Object.entries(gradeCount).map(([grade, count]) => `
                    <span class="px-3 py-1 text-sm font-semibold rounded-full ${getGradeColor(grade)}">
                        ${grade}: ${count}
                    </span>
                `).join('') : `
                    <span class="px-3 py-1 text-sm text-gray-500">No grades available</span>
                `}
            </div>
        </div>
    `;
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <select name="target" required class="w-full p-3 border border-gray-300 rounded-lg">
                                ...
                                <option value="principals" ${notification.target === 'principals' ? 'selected' : ''}>Principals Only</option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-4">
    ...
    const userUnsubscribe = db.collection(USERS_LIST_COLLECTION_PATH).onSnapshot((snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            if (data.role) { 
                ...
                if (data.role === 'principal') principals.push(data);
            }
        });
