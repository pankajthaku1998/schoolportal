...
const trackUserLogin = async (userId) => {
    try {
        ...
        if (userList[userId]) {
            userList[userId].lastLoginTime = now;
            userList[userId].isOnline = true;
            userList[userId].loginCount = (userList[userId].loginCount || 0) + 1;
        }
        
        console.log(`User ${userId} login tracked at ${now}`);
    } catch (error) {
...
// Automatic cleanup function for old login activities
window.cleanupOldLoginActivity = async () => {
    try {
        const now = new Date();
        const oneHourAgo = new Date(now.getTime() - (1 * 60 * 60 * 1000)); // Changed from 24h to 1h
        
        console.log(`Cleaning up login activities older than ${oneHourAgo.toISOString()}`);
        ...
        for (const user of allUsers) {
            ...
            // Check if lastLoginTime is older than 1 hour and user is marked as online
            if (user.lastLoginTime && user.isOnline) {
                const lastLoginDate = new Date(user.lastLoginTime);
                if (lastLoginDate < oneHourAgo) {
                    updates.isOnline = false;
                    updates.lastLogoutTime = now.toISOString();
                    needsUpdate = true;
                    cleanupCount++;
                    console.log(`Marking user ${user.name} as offline due to 1h timeout`);
                }
            }
            ...
            if (needsUpdate) {
                try {
                    ...
                    // Update in teachers or students array
        ...
        if (cleanupCount > 0) {
            console.log(`✅ Cleaned up ${cleanupCount} login activities older than 1 hour`);
            ...
            console.log('✅ No login activities older than 1 hour to clean up');
        }
        
    } catch (error) {
        console.error('Error during login activity cleanup:', error);
    }
};
...
const startLoginActivityCleanup = () => {
    ...
    cleanupInterval = setInterval(window.cleanupOldLoginActivity, 3600000);
    console.log('🕐 Login activity cleanup scheduled every hour (removes activities older than 1 hour)');
};
...
const renderLoginActivity = () => {
    ...
    loginActivityTable.innerHTML = sortedUsers.length > 0 ?
        ...
        '<tr><td colspan="5" class="px-6 py-8 text-center text-gray-500">No login activity found</td></tr>';
